...
router.post('/Get_Reporte_Empresa_Direccion', async (req, res) => {
  try {
    ...
    WHERE EM.NUMERO = '${str_numero}' OR '${str_numero}' = ''`);
    const data = result.recordset.map((row) => ({
      NUMERO: row.NUMERO,
      CLAVE: row.CLAVE,
